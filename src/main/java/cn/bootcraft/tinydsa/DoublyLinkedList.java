package cn.bootcraft.tinydsa;

/**
 * DoublyLinkedList — 带双向哨兵的双向链表 Deque。
 *
 * <p>使用 sentinel head + sentinel tail 双哨兵模式，
 * 每个节点持有 prev 和 next 两个引用，实现两端 O(1) 操作。</p>
 */
public class DoublyLinkedList {

    /** 双向链表节点。 */
    static class DLLNode {
        int val;
        DLLNode prev;
        DLLNode next;

        DLLNode(int val, DLLNode prev, DLLNode next) {
            this.val = val;
            this.prev = prev;
            this.next = next;
        }

        DLLNode() {
            this(0, null, null);
        }
    }

    private final DLLNode head;  // 头哨兵
    private final DLLNode tail;  // 尾哨兵
    private int size;

    public DoublyLinkedList() {
        this.head = new DLLNode();
        this.tail = new DLLNode();
        this.head.next = this.tail;
        this.tail.prev = this.head;
        this.size = 0;
    }

    /** 在头部插入元素。O(1)。 */
    public void addFirst(int val) {
        throw new UnsupportedOperationException("TODO: S03");
    }

    /** 在尾部插入元素。O(1)。 */
    public void addLast(int val) {
        throw new UnsupportedOperationException("TODO: S03");
    }

    /** 删除并返回头部元素。空链表返回 -1。O(1)。 */
    public int removeFirst() {
        throw new UnsupportedOperationException("TODO: S03");
    }

    /** 删除并返回尾部元素。空链表返回 -1。O(1)。 */
    public int removeLast() {
        throw new UnsupportedOperationException("TODO: S03");
    }

    /** 按索引访问元素。越界返回 -1。O(n)。 */
    public int get(int index) {
        throw new UnsupportedOperationException("TODO: S03");
    }

    /** 返回当前元素个数。 */
    public int size() {
        throw new UnsupportedOperationException("TODO: S03");
    }

    /** 将链表元素按正序转为 int 数组。 */
    public int[] toArray() {
        throw new UnsupportedOperationException("TODO: S03");
    }

    /** 将链表元素按反序转为 int 数组（验证 prev 链正确性）。 */
    public int[] toArrayReverse() {
        throw new UnsupportedOperationException("TODO: S03");
    }
}
